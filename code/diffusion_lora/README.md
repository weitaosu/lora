...
Train a Stable Diffusion LoRA on the 271 Bronkhorst images sitting in [data/diffusion_lora/train/](../../data/diffusion_lora/train/).

## Layout

```
final_proj/
├── data/diffusion_lora/train/         <- 271 images, your training set
...
└── models/diffusion_lora/sd15/             <- trained LoRA weights land here
```
...
```bash
cd code/diffusion_lora
python caption_images.py
```

What it does:
- Loads BLIP (Salesforce/blip-image-captioning-large, ~1 GB VRAM)
- For each image in `data/diffusion_lora/train/`, generates a description like "a man riding a wave on a surfboard"
- Prepends the trigger word: `"wbronkhorst style, "`
- Writes `data/diffusion_lora/train/metadata.jsonl` (one JSON line per image — the format diffusers expects)
...
This downloads `train_text_to_image_lora.py` from the official diffusers repo (pinned to v0.30.3 for stability) into `code/diffusion_lora/`.
...
Outputs:
- `models/diffusion_lora/sd15/pytorch_lora_weights.safetensors` — final LoRA weights
- `models/diffusion_lora/sd15/checkpoint-{step}/` — intermediate checkpoints every 500 steps
- `models/diffusion_lora/sd15/logs/` — TensorBoard logs (`tensorboard --logdir models/diffusion_lora/sd15/logs`)
...
Default: generates 6 sample images using built-in Bronkhorst-style prompts (lifeguard, surfer, beach, etc.) → `results/diffusion_lora/sd15_samples/`.
...
```bash
python inference.py --lora ../../models/diffusion_lora/sd15/checkpoint-2000
```
